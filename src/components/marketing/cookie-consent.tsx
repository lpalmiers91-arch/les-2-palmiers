...
export function CookieConsent() {
  ...
  useEffect(() => {
    ...
    try {
      if (!localStorage.getItem(KEY)) t = setTimeout(() => setShow(true), 1600);
    } catch {
      /* stockage indisponible : rien de non essentiel */
    }
    return () => clearTimeout(t);
  }, []);

  function decide(analytics: boolean) {
    const c: Choice = { necessary: true, analytics, at: new Date().toISOString() };
    try {
